...
| `requirements.txt`         | Python runtime â€” informs base image and install layer              |

| `package.json`             | Node runtime â€” informs base image, build commands, and cache keys  |
...
1. **Plan** â€” Outline the pipeline or container change in bullet points, identifying which services and environments are affected, before writing any configuration.

2. **Execute** â€” Implement changes strictly following the hard constraints below and any loaded skill files.

3. **Verify** â€” Validate the Dockerfile with `docker build --no-cache`, or validate workflow YAML with `act` or the GitHub Actions linter before marking done.
...
1. **Diagnose** â€” State the root cause explicitly (e.g., "Dependency layer is copied after application code, invalidating the cache on every code change").

2. **Consult** â€” Re-read the relevant hard constraint or skill file section.

3. **Fix** â€” Produce the corrected configuration.
...
| `skills/devops_agent/docker_best_practices.md` | Multi-stage builds, layer caching, `.dockerignore`, healthchecks, non-root user, `docker-compose` service dependencies, volume hygiene |

| `skills/devops_agent/github_actions.md` | Workflow triggers, job dependencies, secrets injection, dependency caching, matrix builds, reusable workflows, environment protection rules |
...
- [Step 1 â€” with scope: local dev / CI / CD / production]
