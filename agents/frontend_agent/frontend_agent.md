...
| `react` in package.json      | `skills/frontend_agent/react_best_practices.md`     |

| `tailwindcss` in package.json | `skills/frontend_agent/tailwind_design_system.md`  |

| `framer-motion` in package.json | `skills/frontend_agent/framer_motion.md`         |

| `@react-three/fiber` in package.json | `skills/frontend_agent/react_three_fiber.md` |

| `react-hook-form` + `zod` in package.json | `skills/frontend_agent/forms_validation.md` |

| `@sanity/client` in package.json | `skills/frontend_agent/sanity_cms.md`           |

| `lenis` in package.json         | `skills/frontend_agent/smooth-scroll.md`        |

| `gsap` in package.json          | `skills/frontend_agent/gsap-animations.md`      |
...
1. **Plan** â€” Outline your approach in bullet points before writing code (per `AGENTS.md Â§1`).

2. **Execute** â€” Implement changes strictly following the hard constraints below and any loaded skill files.

3. **Verify** â€” Run linters, type checkers, or tests against the changes.
...
- Register GSAP plugins (`ScrollTrigger`, etc.) at module level, never inside component bodies.

- Connect Lenis to the animation library ticker when both smooth scroll and animations are active.

- Use `gsap.context()` for component-scoped GSAP animations and always call `ctx.revert()` in `useEffect` cleanup.
...
1. **Diagnose** â€” State the root cause explicitly.
...
| `skills/frontend_agent/react_best_practices.md` | Functional components, hooks discipline, TypeScript patterns, component file structure |

| `skills/frontend_agent/tailwind_design_system.md` | oklch color vars, CVA variants, `cn()` utility, dark-mode-first conventions |

| `skills/frontend_agent/framer_motion.md` | Animation variants, `AnimatePresence`, performance rules, scroll reveal patterns |

| `skills/frontend_agent/react_three_fiber.md` | Suspense boundaries, `useFrame` performance, dispose on unmount, Drei helpers |

| `skills/frontend_agent/forms_validation.md` | React Hook Form + Zod integration, schema file organization, async submit handling |

| `skills/frontend_agent/sanity_cms.md` | GROQ queries, `sanityClient` singleton, image URL builder, typed response schemas |

| `skills/frontend_agent/smooth-scroll.md` | Lenis initialization, `data-lenis-prevent`, programmatic scroll, ticker integration |

| `skills/frontend_agent/gsap-animations.md` | Scroll-locked sections, odometer counters, ScrollTrigger, `gsap.context()` cleanup |
