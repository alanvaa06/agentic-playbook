...
|----------------------------------------------------------------------|-------------------------------------------------------------|
| `fastapi` in `requirements.txt`                                      | `agents/backend_agent/skills/fastapi_architecture.md`          |
| `psycopg2`, `asyncpg`, or `psycopg` in `requirements.txt`           | `agents/backend_agent/skills/sql_postgres.md`                  |
| `supabase` in any manifest                                           | `agents/backend_agent/skills/supabase_rls.md`                  |
| `alembic` in `requirements.txt` or `prisma` in `package.json`       | `agents/database_agent/skills/migrations.md`                   |
| `pgvector`, `pinecone`, `chromadb`, `qdrant`, or `weaviate` in any manifest | `agents/database_agent/skills/vector_dbs.md`          |
| `stripe` in any manifest                                             | `agents/payment_agent/skills/stripe.md`                       |
| `paypalrestsdk` or `paypal-server-sdk` in any manifest               | `agents/payment_agent/skills/paypal.md`                       |
| `mercadopago` in any manifest                                        | `agents/payment_agent/skills/mercadopago.md`                  |
| `@sanity/client` in `package.json`                                   | `agents/frontend_agent/skills/sanity_cms.md`                   |
| `Dockerfile` or `docker-compose.yml` present                         | `agents/devops_agent/skills/docker_best_practices.md`          |
| `.github/workflows/` directory present                               | `agents/devops_agent/skills/github_actions.md`                 |
...
→ Pass to `agents/agentic_ai_architect/agentic_ai_architect.md` for AI subsystem architecture.
...
|------------|---------|-------------|
| `agents/backend_agent/skills/fastapi_architecture.md` | API contract structure, router layout, dependency injection patterns | `fastapi` in `requirements.txt` |
| `agents/backend_agent/skills/sql_postgres.md` | DB connection layer, async driver choice (asyncpg vs psycopg3), pool config | `psycopg2`, `asyncpg`, or `psycopg` in `requirements.txt` |
| `agents/backend_agent/skills/supabase_rls.md` | Auth model, RLS policy design, realtime architecture | `supabase` in any manifest |
| `agents/database_agent/skills/migrations.md` | Migration strategy in delegation map (Alembic vs Prisma) | `alembic` in `requirements.txt` or `prisma` in `package.json` |
| `agents/database_agent/skills/vector_dbs.md` | Vector storage selection, embedding dimensions, AI subsystem handoff | `pgvector`, `pinecone`, `chromadb`, `qdrant`, or `weaviate` in any manifest |
| `agents/payment_agent/skills/stripe.md` | Payment architecture, webhook endpoint contract | `stripe` in any manifest |
| `agents/payment_agent/skills/paypal.md` | PayPal order flow, IPN webhook contract | `paypalrestsdk` or `paypal-server-sdk` in any manifest |
| `agents/payment_agent/skills/mercadopago.md` | MercadoPago preference flow, IPN webhook contract | `mercadopago` in any manifest |
| `agents/frontend_agent/skills/sanity_cms.md` | CMS content model, GROQ query patterns, frontend data layer | `@sanity/client` in `package.json` |
| `agents/devops_agent/skills/docker_best_practices.md` | Container layout, multi-stage build strategy, service dependencies | `Dockerfile` or `docker-compose.yml` present |
| `agents/devops_agent/skills/github_actions.md` | CI/CD pipeline structure for DevOps Agent delegation | `.github/workflows/` present |
