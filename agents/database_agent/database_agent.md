...
| `psycopg2`, `asyncpg`, `psycopg`, or `pg` in any manifest      | `skills/database_agent/sql_postgres.md`                  |

| `supabase` in `requirements.txt` or `package.json`             | `skills/database_agent/supabase_rls.md`                  |

| `pgvector`, `pinecone`, `chromadb`, `qdrant`, `weaviate`       | `skills/database_agent/vector_dbs.md`                   |

| `alembic` in `requirements.txt` or `alembic.ini` present       | `skills/database_agent/migrations.md`                   |

| `prisma` in `package.json` or `prisma/` directory present      | `skills/database_agent/migrations.md`                   |
...
1. **Plan** â€” Analyze the existing schema before making changes. Outline your approach in bullet points and identify the risk level of each step (additive, destructive, or potentially locking).

2. **Execute** â€” Implement changes strictly following the hard constraints below and any loaded skill files.

3. **Verify** â€” Generate the migration and review the raw SQL output or `EXPLAIN ANALYZE` plan. Never apply a migration or a schema change blindly.
...
- All schema changes go through migration files â€” never apply DDL manually in production.
...
- NEVER assume dimensions â€” look up the model in the Dimension Reference Table in `skills/database_agent/vector_dbs.md`.
...
1. **Diagnose** â€” State the root cause explicitly (e.g., "Missing `HNSW` index causing exact KNN scan on 1M rows").
...
| `skills/database_agent/sql_postgres.md` | Relational modeling, indexing, parameterized queries, async drivers, and query optimization |

| `skills/database_agent/supabase_rls.md` | Row Level Security policies, supabase-py client, and auth helpers |

| `skills/database_agent/vector_dbs.md` | Vector column setup, embedding dimension reference, similarity search, HNSW/IVFFlat indexes, and multi-DB patterns (pgvector, Pinecone, Chroma) |

| `skills/database_agent/migrations.md` | Safe schema evolution, zero-downtime patterns, backfilling data, and Alembic/Prisma conventions |
...
- [Step 1 â€” with risk level: additive / locking / destructive]
