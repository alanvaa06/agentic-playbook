...
|-----------------------------------------------------------------|-------------------------------------------------------------|
| `psycopg2`, `asyncpg`, `psycopg`, or `pg` in any manifest      | `agents/database_agent/skills/sql_postgres.md`                  |
| `supabase` in `requirements.txt` or `package.json`             | `agents/database_agent/skills/supabase_rls.md`                  |
| `pgvector`, `pinecone`, `chromadb`, `qdrant`, `weaviate`       | `agents/database_agent/skills/vector_dbs.md`                   |
| `alembic` in `requirements.txt` or `alembic.ini` present       | `agents/database_agent/skills/migrations.md`                   |
| `prisma` in `package.json` or `prisma/` directory present      | `agents/database_agent/skills/migrations.md`                   |
...
- NEVER assume dimensions — look up the model in the Dimension Reference Table in `agents/database_agent/skills/vector_dbs.md`.
...
|------------|-------------|
| `agents/database_agent/skills/sql_postgres.md` | Relational modeling, indexing, parameterized queries, async drivers, and query optimization |
| `agents/database_agent/skills/supabase_rls.md` | Row Level Security policies, supabase-py client, and auth helpers |
| `agents/database_agent/skills/vector_dbs.md` | Vector column setup, embedding dimension reference, similarity search, HNSW/IVFFlat indexes, and multi-DB patterns (pgvector, Pinecone, Chroma) |
| `agents/database_agent/skills/migrations.md` | Safe schema evolution, zero-downtime patterns, backfilling data, and Alembic/Prisma conventions |
