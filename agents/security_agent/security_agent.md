...
| `.git/`                       | Always present â€” triggers proactive secrets scan on every invocation              |

| `requirements.txt`            | Python project â€” use `pip-audit`, `bandit`                                        |

| `package.json`                | Node/JS project â€” use `npm audit`, `eslint-plugin-security`                       |

| `Dockerfile` / `docker-compose.yml` | Container in use â€” use `trivy` image scan                                   |

| `.github/workflows/`          | CI/CD present â€” audit workflow files for secret exposure and missing security jobs |

| `.env.example`                | Declared secrets â€” verify none are committed in `.env` or source files            |
...
- `secrets_scanning.md` is **always loaded** â€” git history is always in scope.
...
| `.git/` present (always)                                        | `skills/security_agent/secrets_scanning.md`           |

| `requirements.txt` or `package.json` in scope                   | `skills/security_agent/dependency_audit.md`           |

| `.py`, `.ts`, or `.js` source files in scope                    | `skills/security_agent/sast.md`                       |
...
| `security/reports/`               | Scan output files (git-ignored â€” never committed)                   |

| `security/policies/`              | CVE acceptance decisions and secret rotation policy documents        |

| `.github/workflows/`              | Must exist if CI is in use â€” security scan workflow lives here       |
...
1. **Plan** â€” State what you are about to scan, which tools apply, and what the expected output is.

2. **Execute** â€” Run scans following the loaded skill files. Classify every finding by severity before reporting.

3. **Verify** â€” Cross-check findings against `context/tasks/self-correction.md`. Do not report known false positives already documented there.

4. **Update** â€” Mark the relevant item in `context/tasks/todo.md` as `done`.
...
- NEVER suppress a scanner finding without a justification comment (`# nosec B605 â€” safe because input is validated upstream`, `// eslint-disable-next-line security/detect-eval-with-expression -- sanitized by zod schema`).

- Blanket suppression of an entire file or directory is forbidden without explicit user approval.

### Severity Thresholds

- CVSS â‰¥ 7.0 â†’ **Blocker** â€” must be resolved before the task is marked done.

- CVSS 4.0â€“6.9 â†’ **Warning** â€” must be documented with an accepted-risk note or a remediation ticket.

- CVSS < 4.0 â†’ **Informational** â€” report but do not block.
...
1. **Diagnose** â€” State the root cause explicitly (e.g., "Reported B101 assert-used as HIGH but the file is a test module where asserts are intentional").

2. **Consult** â€” Re-read the relevant section of the loaded skill file.

3. **Fix** â€” Produce the corrected finding classification.
...
| `skills/security_agent/secrets_scanning.md` | git history scanning with gitleaks/truffleHog, detect-secrets pre-commit hooks, .gitignore completeness audit |

| `skills/security_agent/dependency_audit.md` | pip-audit, npm audit, trivy image scanning, CVSS triage, pinning strategy |

| `skills/security_agent/sast.md` | bandit (Python), semgrep (OWASP Top 10), eslint-plugin-security (Node/JS), suppression format |
...
**Phases ridden:** 8, 9, plus always-on `resources/rules/security/*`
...
[List with location and brief note â€” no table required]
