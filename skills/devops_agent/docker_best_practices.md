...
If you encounter an unexpected build failure, a bloated image, a container running as root, or any pattern that required more than one attempt to get right, you MUST log the failure to `context/tasks/self-correction.md` before marking the task complete.
