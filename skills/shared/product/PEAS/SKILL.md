...
# PEAS Agent Design â€” Agentic Specification Framework

Produce a structured Agent Design Document that fully specifies an AI agent's behavior, constraints, and evaluation criteria before any architecture or code is written. The output feeds directly into the Agentic AI Architect (`agents/agentic_ai_architect/agentic_ai_architect.md`) for pattern selection and implementation.
...
## Step 1: Discovery Questions â€” Performance

Ask these questions and **STOP AND WAIT** for answers before proceeding to Step 2.

```

## Performance â€” How do we measure success?
...
4. Must the agent explain its reasoning?
   ...
   A. Yes â€” chain-of-thought or citations are required in every response

   B. Partially â€” reasoning for key decisions only

   C. No â€” only the final output matters

```

---

## Step 2: Discovery Questions â€” Environment

Ask these questions and **STOP AND WAIT** for answers before proceeding to Step 3.

```

## Environment â€” What world does this agent operate in?

5. How much of the relevant information can the agent see?

   A. Fully observable â€” the agent has access to all data it needs

   B. Partially observable â€” some information is missing, uncertain, or must be retrieved

   C. Depends on the query â€” sometimes full, sometimes partial

6. Does the environment change while the agent is working?

   A. Static â€” data does not change during a single run

   B. Dynamic â€” external state can change (new messages, updated records, live APIs)

   C. Semi-dynamic â€” data is stable per run but changes between runs

7. How are tasks structured?

   A. Episodic â€” each task is independent; no memory of prior tasks needed

   B. Sequential â€” prior task outcomes affect future decisions

   C. Mixed â€” mostly episodic, but some tasks build on earlier context

8. Does this agent work alone or with others?

   A. Single agent â€” operates independently

   B. Multi-agent coordinator â€” delegates subtasks to other agents

   C. Multi-agent participant â€” receives tasks from a coordinator agent

   D. Peer-to-peer â€” collaborates with agents at the same level

```

---

## Step 3: Discovery Questions â€” Actuators

Ask these questions and **STOP AND WAIT** for answers before proceeding to Step 4.

```

## Actuators â€” What can this agent do?
...
10. Are any actions irreversible or high-risk?
    ...
    A. No â€” all actions can be undone (read-only, or creates new files only)

    B. Yes â€” some actions modify existing data (database writes, file overwrites)

    C. Yes â€” some actions have external side effects (API calls, payments, emails)

    D. Yes â€” destructive actions are possible (delete records, drop tables)

11. Should the agent require human confirmation before acting?

    A. Never â€” fully autonomous

    B. Only for irreversible or high-risk actions (from Q10)

    C. At key checkpoints (e.g., after planning, before executing)

    D. Always â€” every action requires approval

```

---

## Step 4: Discovery Questions â€” Sensors

Ask these questions and **STOP AND WAIT** for answers before proceeding to output generation.

```

## Sensors â€” What inputs does this agent receive?
...
13. How reliable are the inputs?
    ...
    A. Trusted â€” validated by upstream systems, well-formatted

    B. Semi-trusted â€” generally correct but may contain errors or ambiguity

    C. Untrusted â€” raw user input, third-party API responses, noisy data

    D. Mixed â€” depends on the source

14. How does the agent receive inputs?

    A. On-demand â€” user triggers the agent explicitly

    B. Event-driven â€” agent reacts to incoming events or webhooks

    C. Scheduled â€” agent runs on a cron or periodic schedule

    D. Streaming â€” continuous real-time data flow
...
# Agent Design Document â€” [Agent Name]
...
| `skills/agentic_ai_architect/frameworks/[framework]/SKILL.md` | [Framework-specific agent patterns] |

| `skills/agentic_ai_architect/retrieval/[retrieval]/SKILL.md` | [If retrieval is an actuator â€” RAG patterns] |

| `skills/agentic_ai_architect/data/[data]/SKILL.md` | [If multimodal input is a sensor] |

---

## Handoff

â†’ Pass this document to `agents/agentic_ai_architect/agentic_ai_architect.md` to begin architecture and implementation.
